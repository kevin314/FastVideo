...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Compare videos using SSIM/MS-SSIM metrics')
    parser.add_argument('--reference', '-r', type=str, help='Path to reference videos directory')
    parser.add_argument('--generated', '-g', type=str, help='Path to generated videos directory')
    parser.add_argument('--use-ms-ssim', action='store_true', help='Use MS-SSIM instead of SSIM')
    args = parser.parse_args()
    
    script_dir = os.path.dirname(os.path.abspath(__file__))

    reference_folder = args.reference if args.reference else os.path.join(script_dir, 'reference_videos')
    generated_folder = args.generated if args.generated else os.path.join(script_dir, 'generated_videos')
    ...
    results = compare_folders(reference_folder, generated_folder, args.use_ms_ssim)

    print("\n===== SSIM Results Summary =====")
    for video_name, ssim_value in results.items():
        if ssim_value is not None:
            print(f"{video_name}: {ssim_value[0]:.4f}, Min SSIM: {ssim_value[1]:.4f}, Max SSIM: {ssim_value[2]:.4f}")
        else:
