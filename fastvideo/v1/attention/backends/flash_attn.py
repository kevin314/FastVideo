...
# yapf: disable
from fastvideo.v1.attention.backends.abstract import (AttentionBackend, AttentionImpl,
                                              AttentionLayer,
                                              AttentionMetadata,
...
# yapf: enable
from fastvideo.v1.attention.backends.utils import (
...
class TextEncoderFlashAttentionImpl:
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.head_size = head_size
        self.num_kv_heads = num_heads  # For encoder, typically num_kv_heads = num_heads
        ...
        # Always use encoder-only attention type
        self.attn_type = AttentionType.ENCODER_ONLY
        self.vllm_flash_attn_version = get_flash_attn_version()
        
        # Verify head size is supported
        support_head_sizes = FlashAttentionBackend.get_supported_head_sizes()
        if head_size not in support_head_sizes:
            raise ValueError(
                f"Head size {head_size} is not supported by FlashAttention. "
