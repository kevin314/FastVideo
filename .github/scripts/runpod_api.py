...
def parse_arguments():
    ...
    parser.add_argument(
        '--pod-id',
        type=str,
        help='Pod ID for execute or delete actions')
...
def setup_repository(pod_id):
    ...
    try:
        ...
        if return_code != 0:
            ...
            raise RuntimeError(f"Repository setup failed with exit code {return_code}. Output: {stdout_str}")
...
def execute_command(pod_id):
    ...
    ssh_command = [
        ...
        "UserKnownHostsFile=/dev/null", "-o", "ServerAliveInterval=60", "-o",
        "ServerAliveCountMax=10", "-p",
        str(ssh_port), f"root@{ssh_ip}", args.test_command
    ]
